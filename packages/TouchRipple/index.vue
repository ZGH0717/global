<template>
  <!--最外层用div包裹-->
  <div class="" @mousedown="handleMouseDown" @mouseup="end()" @mouseleave="end()" @touchstart="handleTouchStart"
       @touchend="end()" @touchcancel="end()">
    <!--外层包裹防止波纹溢出-->
    <div class="holder" ref="holder" :style="styles">
      <!--多个波纹用 v-for 控制-->
      <circle-ripple v-for="ripple in ripples" :key="ripple.key" :color="ripple.color" :opacity="ripple.opacity"
                     :merge-style="ripple.style"></circle-ripple>
    </div>
    <!--利用slot分发实际内容-->
    <slot></slot>
  </div>
